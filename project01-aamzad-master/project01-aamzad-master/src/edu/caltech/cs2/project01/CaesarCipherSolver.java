...
public class CaesarCipherSolver {
    ...
    public static List<String> splitBySpaces(String s) {
        ArrayList<String> spacedString = new ArrayList<>();
    ...
    public static String putTogetherWithSpaces(List<String> words) {
        ...
        for (int i = 0; i < words.size(); i++) {
            wordsTogether += words.get(i) + " ";
        }
        return wordsTogether.substring(0, wordsTogether.length() - 1);
    }
    ...
    public static void main(String[] args) {
        ...
        List<String> spacedString = splitBySpaces(message);
        for (int i = 0; i < 26; i++) {
            rot(spacedString, 1);
            if (howManyWordsIn(spacedString, dictionary) > spacedString.size() / 2.0) {
                System.out.println((putTogetherWithSpaces(spacedString)));
            }
        }
    }
